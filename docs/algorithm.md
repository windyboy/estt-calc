...
实现上，目标执行日须在季节班期内由 `HistoryFlightProvider` 入口一次性校验；逐条历史仅比较是否与目标**同星期几**（等价于同运营日）。
...
服务提供有界历史查询接口（`GET /estt/history/...`），供运维或调试查看样本，其规则与主估算**不完全相同**：不做到港时刻可信筛选，返回的是经 `passesStageB` 筛选的「基本有效且可比」记录；单次最多读取 `MAX_HISTORY_ROWS` 条原始行，响应含 `rawScanned` 与 `capped`。接口说明见 [api.md](api.md)。

源码对照见 [code-map.md](code-map.md)：`passesStageB`（阶段 B）、`ScheduleDeviation.kt`（阶段 C）。
