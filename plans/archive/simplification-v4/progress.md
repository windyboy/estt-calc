...
### Simplification plan v4 executed (2026-07-01)

User requested: `/planning-with-files 深度简化项目代码，保留充分的代码说明文档`.

**Phase 1 — merge scanner**

- Inlined `HistoryPaginationScanner` into `HistoryFlightProvider`; deleted scanner file (−1 production file).

**Phase 2 — slim EsttService**

- Added `catching` helper; inlined calculation telemetry; removed `isOperationDayMatch` wrapper.
- `EsttService.kt`: 342 → 263 lines.

**Phase 3 — test deduplication**

- Added `EsttServiceTestSupport.kt`; refactored four EsttService test classes.
- Removed duplicate `isOperationDayMatch` describe block from `EsttServiceTest`.
- Merged `EsttCalcTest` into `ApplicationTest`.

**Phase 4 — docs restructure**

- Created `docs/algorithm.md`, `docs/api.md`, `docs/code-map.md`.
- Slimmed README (~820 → ~120 lines); content preserved in docs.

**Phase 5 — verify**

```bash
./gradlew spotlessApply check   # pass
```

| Metric | v3 | v4 |
|--------|-----|-----|
| Production files | 24 | 23 |
| Main Kotlin lines | 1,477 | 1,348 |
| `EsttService.kt` | 342 | 263 |

All v4 phases complete in `task_plan.md`.
