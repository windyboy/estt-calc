...
# Findings: Simplification plan v4 (2026-07-01)

## Before / after

| Metric | v3 (baseline) | v4 (after) | Δ |
|--------|---------------|------------|---|
| `EsttService.kt` | 342 | 263 | −79 |
| `HistoryFlightProvider.kt` | 143 | 172 | +29 (scanner inlined) |
| Production `.kt` files | 24 | 23 | −1 |
| Main Kotlin lines | 1,477 | 1,348 | −129 (~8.7%) |
| README lines | ~820 | ~120 | moved to `docs/` |

## Changes delivered

- **Phase 1:** Deleted `HistoryPaginationScanner.kt`; pagination scan is private `scanPaginatedHistory` in provider.
- **Phase 2:** `EsttService.catching` for Result wrappers; inlined `withCalculationTelemetry`; direct `OperationDays.matches`; EsttService 342→263 lines.
- **Phase 3:** `EsttServiceTestSupport.kt` shared fixtures; removed duplicate `beforeEach` and `createHistoryFlights`; removed redundant `isOperationDayMatch` tests (covered by `OperationDaysTest`); merged disabled Micronaut test into `ApplicationTest`.
- **Phase 4:** Added `docs/algorithm.md`, `docs/api.md`, `docs/code-map.md`; slim README with documentation index; updated `docs/development.md` links.

## Documentation policy (v4)

- README: overview, quick start, config table, links — not algorithm/API essays.
- `docs/`: full algorithm, API contract, source navigation.
- Source: bilingual domain comments preserved at non-obvious rules (unchanged policy from v3).

## Verification

```bash
./gradlew check   # pass (2026-07-01, after spotlessApply)
```
